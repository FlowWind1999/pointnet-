...
class PartNormalDataset():
    ...
    def __getitem__(self, index):
        if index in self.cache:
            ...
            normal = data[:,3:6]  #我们的数据集没有这个部分  normal是法向量
            seg = data[:,-1].astype(np.int32)
            if len(self.cache) < self.cache_size:
                self.cache[index] = (point_set, normal, seg, cls)
                
        
        choice = np.random.choice(len(seg), self.npoints, replace=True)   #抽样
        #resample
...
if __name__ == '__main__':
    ...
    d = PartNormalDataset(root = '../data/ours', split='trainval', npoints=6000,classification=False)
    print('len_d',len(d))
    ...
    d = PartNormalDataset(root = '../data/ours', classification = True)
    print(len(d))
    ps, normal, cls = d[0]
