...
def eval_one_epoch(sess, ops):
    ...
    for batch_idx in range(num_batches):
        ...
        #召回率
        bentch=np.zeros(cur_pred_val.shape)
        zero_num=np.sum(cur_pred_val==bentch)
        recall_num=np.sum((cur_pred_val==bentch)&(cur_pred_val == cur_batch_label))
        total_recall_num+=recall_num
        total_zero_num+=zero_num
    ...
    recall=total_recall_num/float(total_zero_num)
    log_string('eval recall : {}'.format(total_recall_num/float(total_zero_num)))

    F1_score=2*recall*accuracy(accuracy+recall)
    log_string('eval F1_score : {}'.format(F1_score))
